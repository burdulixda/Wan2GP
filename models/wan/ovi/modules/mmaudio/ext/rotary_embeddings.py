...
def compute_rope_rotations(length: int,
    ...
    with torch.amp.autocast(device_type=get_accelerator_type(device), enabled=False):
...
def apply_rope(x: Tensor, rot: Tensor) -> tuple[Tensor, Tensor]:
    with torch.amp.autocast(device_type=get_accelerator_type(x.device), enabled=False):
        _x = x.float()
